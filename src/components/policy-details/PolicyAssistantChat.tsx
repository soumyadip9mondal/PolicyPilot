...
import { Bot, Send, Info, AlertCircle, RefreshCw } from "lucide-react";
...
const SYSTEM_PROMPT = `You are PolicyPilot AI, an expert policy assistant. You help users understand government policies, scholarships, and insurance schemes. 
When answering:
- Be concise and clear (3-5 sentences max per response)
- Focus on practical guidance
- Highlight important conditions or warnings
- If you don't have enough context, ask the user for more details about the specific policy`;

const SUGGESTED_QUERIES = [
    "What documents are required?",
    "Am I eligible for this policy?",
    "Explain the age limit conditions",
    "What are the hidden restrictions?",
];
...
export function PolicyAssistantChat({ policyContext }: PolicyAssistantChatProps) {
    const [messages, setMessages] = useState<Message[]>([
        {
            role: "assistant",
            content:
                "Hello! I'm your PolicyPilot AI assistant. Upload a policy document above and I'll analyze it for you, or ask me any question about policies, eligibility, or required documents.",
        },
    ]);
    ...
    const sendMessage = async (text?: string) => {
        ...
        try {
            ...
            if (data.error) {
                ...
                logEvent("AI Help", "Policy Assistant Chat", userText.slice(0, 60), "resolved");
            }
        } catch {
    ...
    return (
        <div className="bg-card rounded-2xl shadow-soft border border-border h-[600px] flex flex-col">
            {/* Header */}
            <div className="p-4 border-b border-border flex items-center justify-between flex-shrink-0">
                ...
                <button className="text-muted-foreground hover:text-foreground transition-colors p-2" title="About">
                    <Info size={18} />
                </button>
            </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto bg-muted/10 space-y-4">
                ...
                {/* Suggested queries – show only after initial message */}
                {messages.length === 1 && (
            ...
            <div className="p-4 border-t border-border bg-card rounded-b-2xl flex-shrink-0">
                <div className="relative flex items-center">
                    ...
                    <button
                        ...
                        className="absolute right-2 w-8 h-8 bg-primary text-primary-foreground rounded-full flex items-center justify-center hover:bg-primary/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    >
